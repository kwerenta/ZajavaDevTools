...
const Message = props => {
    ...
    return (
        <ListItem onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            <Grid
                container
                direction={dialogue.sender == "player" ? "row-reverse" : "row"}
                justify="flex-start"
                alignItems="center"
            >
                {(dialogue.sender == "narrator" || dialogue.sender == "stage") && <Grid item xs={3} />}

                <Grid
                    item
                    xs={(dialogue.sender == "narrator" || dialogue.sender == "stage") && 6}
                    className={clsx(
                        classes.message,
                        dialogue.sender == "npc"
                            ? classes.textNpc
                            : dialogue.sender == "player"
                            ? classes.textPlayer
                            : dialogue.sender == "narrator"
                            ? classes.textNarrator
                            : classes.textStage
                    )}
                >
                    <ListItemText
                        align={dialogue.sender == "narrator" || dialogue.sender == "stage" ? "center" : "left"}
                    ...
                    >
                        <Grow in={options} timeout={500}>
                            <Tooltip title="Edytuj wiadomość" arrow>
                                <IconButton
                                    onClick={() => {
                                        props.handleClickEdit(dialogue.id);
                                    }}
                                    size="small"
                                >
                                    <EditIcon />
                        ...
                        <Grow in={options} timeout={500}>
                            <Tooltip title="Usuń wiadomość" arrow>
                                <IconButton
                                    onClick={() => {
                                        props.handleClickDelete(dialogue.id);
                                    }}
                                    size="small"
                                >
                                    <DeleteIcon />
...
const Chat = props => {
    ...
    const [form, setForm] = useState({
        sender: "",
        text: "",
        name: props.quest && props.quest.name,
    });
    ...
    const addMessage = async () => {
        if (form.sender && form.text) {
            ...
            setSnack({
                open: true,
                severity: "error",
                text: "Pole mówca oraz tekst nie mogą być puste!",
            });
        }
    };
    const editMessage = () => {
    ...
    return (
        <>
                    ...
                    <List>
                        <ListItem key="NPC">
                            <ListItemIcon>
                                <Avatar
                                    alt="NPC"
                                    src="https://i1.sndcdn.com/avatars-000671349956-8oskmm-t500x500.jpg"
                                />
                            </ListItemIcon>
                        ...
                        <Grid item xs={3}>
                            <FormControl disabled={questStatus.completed} variant="outlined" fullWidth>
                                ...
                                >
                                    ...
                                    <MenuItem value="stage">Etap</MenuItem>
                                </Select>
                            </FormControl>
                        </Grid>
                        <Grid item xs={8}>
            ...
            <Confirmation
                ...
                text={
                    edit
                        ? "Czy na pewno chcesz zedytować tę kwestię? Operacji nie będzie można już cofnąć."
